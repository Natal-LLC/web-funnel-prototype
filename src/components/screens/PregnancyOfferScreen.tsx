...
interface PregnancyOfferScreenProps {
  programs?: string;
  trimester?: string;
}

export function PregnancyOfferScreen({ programs, trimester }: PregnancyOfferScreenProps) {
  ...
  const getTrimesterText = () => {
    if (trimester === 'first') return 'first trimester';
    if (trimester === 'second') return 'second trimester';
    if (trimester === 'third') return 'third trimester';
    return 'pregnancy journey';
  };

  const getProgramText = () => {
    if (!programs) return 'pregnancy programs';
    
    const programNames = programs.split(',').map(programId => {
      const programMap: Record<string, string> = {
        'ea-1t': 'Expecting Athletes First Trimester',
        'ea-2t': 'Expecting Athletes Second Trimester', 
        'ea-3t': 'Expecting Athletes Third Trimester',
        'beginner-bump': 'Beginner Bump',
        'ab-prehab': 'Ab Prehab',
        'labor-prep': 'Labor Prep'
      };
  ...
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Clear errors when user starts typing valid email
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    // Comprehensive validation for subscription
    ...
    try {
      ...
      window.location.href = `/pregnancy-recommendations?${params.toString()}`;
    } catch (error) {
            ...
            <div className="flex flex-col h-full justify-center">
              <div className="space-y-4">
                ...
                <div className="mt-4">
                  ...
                  <form onSubmit={handleEmailSubmit} className="mt-1 space-y-1">
                    <div>
                      <label htmlFor="email" className="block text-xs font-medium text-foreground mb-0.5">
                        Continue with Email
                      </label>
                      <input
                        type="email"
                        ...
                        className={`w-full px-3 py-1.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent ${
                    ...
                    <button
                      ...
                      className="w-full bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed px-6 py-1.5 rounded-lg text-sm font-medium transition-colors"
                    >
